...
def test_access_without_configuration(request):
...
def test_access_from_localhost_development_mode(request):
...
def test_access_from_localhost_production_mode(request):
...
def test_access_from_gb_when_gb_in_countries_whitelist(request):
    """Access is granted from GB when GB is in the counties' whitelist."""
    assert grants_access(request, "212.102.63.59")


@override_settings(WHITELIST_COUNTRIES=["US"])
def test_access_from_gb_when_gb_not_in_countries_whitelist(request):
...
def test_mix_config_access_from_gb_when_eu_in_whitelist_but_gb_is_forbidden(request):
...
def test_mix_config_access_from_gb_when_gb_in_both_lists(request):
